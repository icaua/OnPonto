(function (root) {
  ...
  function demoBanner() {
    return [
      ...
      '<span><strong>Modo demonstração</strong> · Dados simulados. Nenhuma informação será enviada ao backend.</span>',
      "</div>",
    ].join("");
  }
  ...
  function actionButton(action) {
    ...
    return '<button class="button ' + escapeHtml(item.variant || "button--secondary") + '" type="button"' + actionAttribute + routeAttribute + extra + '>' + escapeHtml(item.label || "Ação") + "</button>";
  }
  ...
  function renderCompanies(state, data, components) {
    ...
    var rows = visibleCompanies.map(function (company) {
      ...
      if (current) employeeCount = valueOf(current, ["employeeCount", "quantidadeFuncionarios"], employeeCount);
    ...
    var table = rows ? '<div class="table-frame"><table class="data-table"><thead><tr><th>Empresa</th><th>Competência atual</th><th>Status</th><th>Funcionários</th><th>Pendências</th><th>Última atualização</th><th><span class="sr-only">Ação</span></th></tr></thead><tbody>' + rows + "</tbody></table></div>" : emptyState(context.components, "Nenhuma empresa encontrada", search ? "Tente outro termo de busca." : "Crie uma empresa para iniciar o trabalho.", { label: "Nova empresa", action: "new-company" });
    return [
  ...
  function renderCompanyOverview(state, data, components) {
    ...
    var activeEmployeeCount = current ? valueOf(current, ["employeeCount", "quantidadeFuncionarios"], companyEmployees.length) : companyEmployees.length;
    ...
    return [
      ...
      pageHeader(context.components, { title: companyName(company), subtitle: valueOf(company, ["legalName", "razaoSocial"], "Visão geral da empresa"), breadcrumbs: companyBreadcrumbs(company) }),
  ...
  function renderCompanyCompetencies(state, data, components) {
    ...
    return [
      ...
      '<div class="toolbar"><div class="filter-group" role="group" aria-label="Situação">' + filterButton("Todas", "all", statusFilter, "competencies") + filterButton("Abertas", "aberta", statusFilter, "competencies") + filterButton("Em conferência", "em_conferencia", statusFilter, "competencies") + filterButton("Fechadas", "fechada", statusFilter, "competencies") + "</div></div>",
      '<section class="work-card">' + (rows ? '<div class="table-frame"><table class="data-table"><thead><tr><th>Competência</th><th>Status</th><th>Funcionários</th><th>Progresso</th><th>Pendências</th><th>Arquivos</th><th>Atualização</th><th><span class="sr-only">Ação</span></th></tr></thead><tbody>' + rows + "</tbody></table></div>" : emptyState(context.components, "Nenhuma competência encontrada", "Ajuste o filtro ou crie uma nova competência.", { label: "Nova competência", action: "new-competence" })) + "</section></section>",
    ].join("");
  }
  ...
  function importAnalysisCard(context, analysis) {
    ...
    return [
      ...
      '<div class="section-heading"><div><span class="eyebrow">Análise simulada concluída</span><h2 id="analysisTitle">' + escapeHtml(valueOf(analysis, ["fileName", "nomeArquivo"], "Arquivo de ponto")) + "</h2></div>" + statusChip(context.components, "analisado", "Pronto para prévia") + "</div>",
  ...
  function renderImports(state, data, components) {
    ...
    var fileTypes = collection(context.data, ["fileTypeOptions", "tiposArquivo"]);
    if (!fileTypes.length) {
      fileTypes = [
    ...
    var dropzone = component(context.components, ["ImportDropzone", "importDropzone"], {
      ...
      accept: ".txt,.xls,.xlsx,.pdf,.png,.jpg,.jpeg",
      ...
      return '<label class="import-dropzone" for="importFile" data-action="open-file-picker"><span class="import-dropzone__icon" aria-hidden="true">⇧</span><strong>Arraste o arquivo aqui</strong><span>ou escolha um arquivo do computador</span><small>TXT, XLS, XLSX, PDF, PNG ou JPG · demonstração sem envio</small><input id="importFile" name="importFile" type="file" accept=".txt,.xls,.xlsx,.pdf,.png,.jpg,.jpeg" data-action="select-import-file"></label>';
    });
    ...
    return [
      ...
      '<form class="work-card import-form" data-form="import" data-action="analyze-import">',
      ...
      fileTypes.map(function (type, index) {
        ...
        return '<label><input type="radio" name="importType" value="' + escapeHtml(typeValue) + '" data-action="select-import-type"' + (checked ? " checked" : "") + "><span>" + escapeHtml(valueOf(type, ["label", "nome"], typeValue)) + "</span></label>";
      }).join(""),
      "</div></fieldset>",
      dropzone,
      '<div class="form-footer"><p class="helper-text">O botão apenas simula a análise. O arquivo não será salvo.</p><button class="button button--primary" type="submit" data-action="analyze-import">Analisar arquivo</button></div>',
      "</form>",
  ...
  function renderImportPreview(state, data, components) {
    ...
    var analysis = valueOf(context.state, ["importAnalysis", "analiseImportacao", "analysisResult"], null) || valueOf(context.data, ["importAnalysis", "analiseImportacao"], {}) || {};
    ...
    if (!rows.length) rows = collection(context.data, ["importPreviewRows", "linhasPrevia"]);
    ...
    var table = visibleRows.length ? [
      ...
      "<thead><tr><th>Funcionário</th><th>Data</th><th>Batidas originais</th><th>Interpretação sugerida</th><th>Status</th><th>Observação</th></tr></thead><tbody>",
      visibleRows.map(function (row) {
        var punches = originalPunchValues(row);
        return [
          '<tr data-preview-row-id="' + escapeHtml(idOf(row)) + '">',
          "<td><strong>" + escapeHtml(valueOf(row, ["employeeName", "funcionario", "nomeFuncionario"], "—")) + "</strong></td>",
          "<td>" + escapeHtml(formatDate(valueOf(row, ["date", "data"], null), true)) + "</td>",
          ...
          "<td>" + escapeHtml(valueOf(row, ["observation", "observacao"], "—") || "—") + "</td>",
          "</tr>",
        ].join("");
      }).join(""),
      "</tbody></table></div>",
    ...
    return [
      ...
      competencyAreaNavigation("imports"),
      '<section class="work-card import-summary"><div class="section-heading"><div><span class="eyebrow">Arquivo analisado</span><h2>' + escapeHtml(fileName) + '</h2></div><span class="unsaved-badge">Não salvo</span></div>',
      '<dl class="summary-grid summary-grid--seven">',
      ...
      '</div><label class="compact-field"><span>Funcionário</span><select data-action="filter-preview-employee"><option value="">Todos</option>' + selectOptions(entities.companyEmployees, employeeFilter, employeeName) + "</select></label></div>",
      table,
      '<footer class="screen-actions screen-actions--sticky"><button class="button button--ghost" type="button" data-route="imports" data-action="navigate">Voltar</button><div><button class="button button--danger-ghost" type="button" data-action="discard-import"' + (hasAnalysis ? "" : " disabled") + '><span>Descartar</span></button><button class="button button--primary" type="button" data-action="save-import-start-review" data-route="review"' + (rows.length ? "" : " disabled") + '>Salvar importação e iniciar conferência</button></div></footer>',
      "</section>",
    ].join("");
  }
  ...
  function renderCompetencies(state, data, components) {
    ...
    return [
      ...
      filterButton("Todas", "all", statusFilter, "competencies"), filterButton("Abertas", "aberta", statusFilter, "competencies"), filterButton("Em conferência", "em_conferencia", statusFilter, "competencies"), filterButton("Fechadas", "fechada", statusFilter, "competencies"),
      "</div></div>",
      '<section class="work-card">' + table + "</section></section>",
    ].join("");
  }
  ...
  function activityTimeline(context, items) {
    return component(context.components, ["ActivityTimeline", "activityTimeline"], { items: items, activities: items }, function () {
      if (!items.length) return emptyState(context.components, "Histórico vazio", "As atividades desta competência aparecerão aqui.");
  ...
  function renderCompetencySummary(state, data, components) {
    ...
    return [
      ...
      pageHeader(context.components, {
        ...
        actions: [
          { label: "Adicionar arquivos", variant: "button--secondary", route: "imports", action: "navigate" },
          { label: "Continuar conferência", variant: "button--primary", route: "review", action: "navigate" },
          { label: "Exportar", route: "competency-exports", action: "navigate" },
          { label: "Fechar competência", variant: "button--danger-ghost", action: "request-close-competence" },
        ],
      }),
      ...
      '</dl></div><section class="subsection"><div class="section-heading"><div><h2>Atividade recente</h2><p>O histórico preserva o caminho do original até a conferência.</p></div><button class="text-button" type="button" data-action="navigate" data-route="competency-history">Ver histórico completo</button></div>' + activityTimeline(context, activities.slice(0, 4)) + "</section></section></section>",
    ].join("");
  }
  ...
  function renderCompetencyFiles(state, data, components) {
    ...
    return '<section class="screen screen--competency-files" data-screen="competency-files">' + demoBanner() + pageHeader(context.components, {
      ...
      actions: [{ label: "Adicionar arquivos", variant: "button--primary", route: "imports", action: "navigate" }],
    }) + competencyAreaNavigation("competency-files") + '<section class="work-card">' + (rows ? '<div class="table-frame"><table class="data-table"><thead><tr><th>Arquivo</th><th>Formato</th><th>Recebido em</th><th>Registros</th><th>Pendências</th><th><span class="sr-only">Ação</span></th></tr></thead><tbody>' + rows + "</tbody></table></div>" : emptyState(context.components, "Nenhum arquivo recebido", "Adicione documentos para iniciar a importação.", { label: "Adicionar arquivos", route: "imports", action: "navigate" })) + "</section></section>";
  }
  ...
  function renderCompetencyHistory(state, data, components) {
    ...
    }) + competencyAreaNavigation("competency-history") + '<section class="work-card">' + activityTimeline(context, activities) + "</section></section>";
  }
  ...
  function editableTimeCell(context, day, field, value, label) {
    return '<input class="editable-time-cell" type="text" inputmode="numeric" autocomplete="off" spellcheck="false" value="' + escapeHtml(value || "") + '" placeholder="—" aria-label="' + escapeHtml(label + " de " + formatDate(dayDate(day), true)) + '" data-action="edit-time" data-day-id="' + escapeHtml(idOf(day)) + '" data-field="' + escapeHtml(field) + '">';
  }

  function fallbackAttendanceTable(context, days, selectedDay, selectedIds) {
    ...
    return [
      ...
      days.map(function (day) {
        ...
        return [
          ...
          '<td class="select-column"><input type="checkbox" data-action="toggle-day-selection" data-day-id="' + escapeHtml(idOf(day)) + '" aria-label="Selecionar ' + escapeHtml(formatDate(dayDate(day), true)) + '"' + (checked ? " checked" : "") + "></td>",
          ...
          "<td>" + editableTimeCell(context, day, "entry", slots.entry, "Entrada") + "</td>",
          "<td>" + editableTimeCell(context, day, "breakStart", slots.breakStart, "Saída do intervalo") + "</td>",
          "<td>" + editableTimeCell(context, day, "breakEnd", slots.breakEnd, "Retorno do intervalo") + "</td>",
          "<td>" + editableTimeCell(context, day, "exit", slots.exit, "Saída") + "</td>",
          ...
          '<td><input class="observation-cell" type="text" value="' + escapeHtml(valueOf(day, ["observation", "observacao"], "")) + '" aria-label="Observação de ' + escapeHtml(formatDate(dayDate(day), true)) + '" data-action="edit-observation" data-day-id="' + escapeHtml(idOf(day)) + '"></td>',
          "</tr>",
        ].join("");
      }).join(""),
      "</tbody></table></div>",
  ...
  function fallbackDayDetailsPanel(context, day) {
    ...
    return [
      ...
      '<section class="context-actions"><h3>Ações</h3><div class="context-actions__buttons">' + contextActions(day).map(function (action) {
  ...
  function renderReview(state, data, components) {
    ...
    var selection = asArray(valueOf(context.state, ["selectedDayIds", "bulkSelection", "diasSelecionados"], []));
    ...
    var table = component(context.components, ["AttendanceTable", "attendanceTable"], {
      days: days,
      rows: days,
      selectedDayId: selectedDay ? idOf(selectedDay) : null,
      selectedIds: selection,
      bulkSelection: selection,
    }, function () { return fallbackAttendanceTable(context, days, selectedDay, selection); });
    ...
    }, function () { return fallbackDayDetailsPanel(context, selectedDay); });
    ...
    return [
      ...
      pageHeader(context.components, {
        ...
        actions: [{ label: "Salvar agora", action: "save-now" }],
      }),
      ...
      '<div class="review-progress">' + progressIndicator(context.components, confirmedDays, eligibleDays, "Progresso") + autosaveIndicator(context) + "</div>",
      ...
      "</section>",
      '<section class="bulk-actions' + (selection.length ? " is-active" : "") + '" aria-label="Ações em massa" aria-live="polite"><strong>' + selection.length + " registro" + (selection.length === 1 ? "" : "s") + ' selecionado' + (selection.length === 1 ? "" : "s") + '</strong><div><button class="button button--secondary" type="button" data-action="bulk-confirm" data-bulk-action="confirm"' + (selection.length ? "" : " disabled") + '>Marcar como conferidos</button><button class="button button--secondary" type="button" data-action="bulk-set-status" data-bulk-action="set_status"' + (selection.length ? "" : " disabled") + '>Definir situação</button><button class="button button--secondary" type="button" data-action="bulk-add-observation" data-bulk-action="add_observation"' + (selection.length ? "" : " disabled") + ">Adicionar observação</button></div><small>As batidas originais nunca serão alteradas.</small></section>",
  ...
  function renderOcr(state, data, components) {
    ...
    return [
      ...
      '<footer class="screen-actions"><button class="button button--ghost" type="button" data-route="imports" data-action="navigate">Voltar</button><button class="button button--primary" type="button" data-action="accept-ocr-preview"' + (rows.length ? "" : " disabled") + '>Continuar para prévia</button></footer></section>',
      "</div></section>",
    ].join("");
  }
  ...
  function competencyExportContent(context, entities) {
    var competence = entities.competence;
    var pending = Number(valueOf(competence, ["pendingCount", "quantidadePendencias"], 0));
    return [
      ...
      reportCard("Planilha de conferência", "Exportação operacional com as marcações e o resultado atual da conferência.", "Exportar Excel", "export-excel"),
      ...
      "</div>",
      '<section class="work-card report-readiness"><div class="section-heading"><div><h2>Prontidão para exportação</h2><p>Os indicadores abaixo são simulados.</p></div>' + statusChip(context.components, pending ? "conferir" : "normal", pending ? "Revisão recomendada" : "Pronto para exportar") + '</div><dl class="summary-grid summary-grid--four">' + summaryItem("Empresa", companyName(entities.company)) + summaryItem("Competência", formatCompetence(competence)) + summaryItem("Progresso", valueOf(competence, ["progress", "progresso"], 0) + "%") + summaryItem("Pendências", pending) + "</dl></section>",
    ].join("");
  }
  ...
  function reportCard(title, description, label, action) {
    return '<article class="report-card"><span class="report-card__icon" aria-hidden="true">▤</span><h2>' + escapeHtml(title) + "</h2><p>" + escapeHtml(description) + '</p><button class="button button--secondary" type="button" data-action="' + escapeHtml(action) + '">' + escapeHtml(label) + "</button></article>";
  }
  ...
  function renderRegistrations(state, data, components) {
    ...
    return [
      ...
      pageHeader(context.components, { title: "Cadastros", subtitle: "Mantenha empresas e funcionários usados na demonstração.", actions: [{ label: activeTab === "employees" ? "Novo funcionário" : "Nova empresa", variant: "button--primary", action: activeTab === "employees" ? "new-employee" : "new-company" }] }),
  ...
  function renderCompanySettings(state, data, components) {
    ...
    return [
      ...
      pageHeader(context.components, { title: "Configurações", subtitle: companyName(company) + " · Preferências locais do protótipo.", breadcrumbs: companyBreadcrumbs(company, "Configurações") }),
      '<div class="settings-layout">',
      '<section class="work-card settings-section"><div class="section-heading"><div><h2>Jornada padrão</h2><p>Usada apenas para calcular saldos simulados na interface.</p></div></div><dl class="detail-grid">' + summaryItem("Empresa", companyName(company)) + '</dl><div class="field-grid field-grid--two"><label class="field"><span>Segunda a sexta</span><input type="text" inputmode="decimal" value="08:00" data-action="change-weekday-schedule"></label><label class="field"><span>Sábado</span><input type="text" inputmode="decimal" value="04:00" data-action="change-saturday-schedule"></label></div><div class="field-grid field-grid--two"><label class="field"><span>Tolerância de atraso</span><div class="input-suffix"><input type="number" min="0" value="5" data-action="change-late-tolerance"><span>min</span></div></label><label class="field"><span>Tolerância de saldo positivo</span><div class="input-suffix"><input type="number" min="0" value="10" data-action="change-extra-tolerance"><span>min</span></div></label></div></section>',
      '<section class="work-card settings-section"><div class="section-heading"><div><h2>Experiência de conferência</h2><p>Preferências salvas somente neste navegador.</p></div></div><label class="switch-row"><span><strong>Salvamento automático simulado</strong><small>Exibe o estado de salvamento após cada edição.</small></span><input type="checkbox" role="switch"' + (valueOf(settings, ["autosave"], true) ? " checked" : "") + ' data-action="toggle-autosave"></label><label class="switch-row"><span><strong>Alertar horários incomuns</strong><small>Permite o horário e pede uma confirmação adicional.</small></span><input type="checkbox" role="switch"' + (valueOf(settings, ["unusualTimeAlerts"], true) ? " checked" : "") + ' data-action="toggle-unusual-time-alert"></label><label class="switch-row"><span><strong>Atalhos de uma tecla</strong><small>Desativados automaticamente durante edição de texto.</small></span><input type="checkbox" role="switch"' + (valueOf(settings, ["singleKeyShortcuts"], true) ? " checked" : "") + ' data-action="toggle-single-key-shortcuts"></label></section>',
      '<section class="work-card settings-section settings-section--demo"><span class="eyebrow">Ambiente atual</span><h2>Modo demonstração</h2><p>Todos os dados são fictícios. A interface não chama a API e não altera arquivos reais.</p><dl class="detail-grid">' + summaryItem("Empresa ativa", companyName(company)) + summaryItem("Armazenamento", "Memória do navegador") + summaryItem("Integração", "Desativada") + '</dl><button class="button button--danger-ghost" type="button" data-action="reset-demo-data">Restaurar dados simulados</button></section>',
      "</div></section>",
    ].join("");
  }
