...
def validar_vinculos(db: Session, competencia_id: int, funcionario_id: int) -> None:
...
def salvar_marcacao(payload: MarcacaoCreate, db: Session = Depends(get_db)) -> MarcacaoPonto:
    ...
    validar_vinculos(db, payload.competencia_id, payload.funcionario_id)
...
def atualizar_marcacao(
    ...
    validar_vinculos(db, competencia_id, funcionario_id)
